<%@ Page Title="" Language="C#" MasterPageFile="~/Main.Master" AutoEventWireup="true" CodeBehind="EduVideoInsert.aspx.cs" Inherits="Assignment.EduVideoInsert" %>
...
&nbsp;&nbsp;&nbsp;
    ...
    <p>
        ...
        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ServicesContext %>" SelectCommand="SELECT * FROM [EducationVideo] WHERE ([Category] = @Category)">
            <SelectParameters>
                <asp:ControlParameter ControlID="ddlSort" Name="Category" PropertyName="SelectedValue" Type="String" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:ServicesContext %>" SelectCommand="SELECT * FROM [EducationVideo]" DeleteCommand="DELETE FROM [EducationVideo] WHERE [VidID] = @VidID" InsertCommand="INSERT INTO [EducationVideo] ([Category], [Description], [Path]) VALUES (@Category, @Description, @Path)" UpdateCommand="UPDATE [EducationVideo] SET [Category] = @Category, [Description] = @Description, [Path] = @Path WHERE [VidID] = @VidID">
            <DeleteParameters>
